...
November 2025 – FULL Indicator Version
"""
...
def compute_indicators(df):
    ...
    ema50 = EMAIndicator(c, window=50).ema_indicator().iloc[-1] if len(c) >= 50 else None
    ema200 = EMAIndicator(c, window=200).ema_indicator().iloc[-1] if len(c) >= 200 else None
    bb = BollingerBands(c, window=20, window_dev=2)
    bb_lower, bb_upper, bb_mavg = bb.bollinger_lband(), bb.bollinger_hband(), bb.bollinger_mavg()
    bb_pct = ((c.iloc[-1] - bb_lower.iloc[-1]) / (bb_upper.iloc[-1] - bb_lower.iloc[-1])) if (bb_upper.iloc[-1] - bb_lower.iloc[-1]) != 0 else 0
    bb_signal = "Overbought 🔴" if bb_pct > 0.8 else "Oversold 🟢" if bb_pct < 0.2 else "Neutral"
    bb_width = (bb_upper.iloc[-1] - bb_lower.iloc[-1]) / bb_mavg.iloc[-1] if bb_mavg.iloc[-1] != 0 else 0
...
def main():
    ...
    fields = [
        ...
        {"name": "📉 MACD", "value": f"{fmt(i['macd'],4)} (signal {fmt(i['signal'],4)}) ({i['macd_div']}) | {i['hist_trend']}", "inline": True},
        {"name": "📊 EMA50 / EMA200", "value": f"{fmt(i['ema50'],4)} / {fmt(i['ema200'],4)}", "inline": True},
        {"name": "📊 Bollinger", "value": f"Middle: {fmt(i['bb_mavg'],4)} | Width: {fmt(i['bb_width'],4)} | Signal: {i['bb_signal']}", "inline": True},
    ...
    send_discord_embed("XRP Combined Intel Report", fields, news)
    post_to_x(f"XRP Intel Report | Price: ${fmt(price,3)} | RSI: {fmt(i['rsi'],2)} | MACD: {fmt(i['macd'],4)} | Structure: {market_struct} | Triggers: {triggers}")
