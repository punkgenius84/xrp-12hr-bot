...
DISCORD_WEBHOOK = os.getenv("DISCORD_WEBHOOK")  # crypto intel portion
WEBHOOK_URL = os.getenv("WEBHOOK_URL")          # xrp-report portion
DISCORD_CHUNK_LIMIT = 1800  # safe chunk size for Discord messages
COINGECKO_MARKETS_URL = "[https://api.coingecko.com/api/v3/coins/markets](https://api.coingecko.com/api/v3/coins/markets)"
STABLECOIN_IDS = ["tether", "usd-coin", "binance-usd", "dai", "true-usd"]

CSV_FILE = "xrp_history.csv"

# XRP detection parameters
...
level = None
if ratio >= VOLUME_SPIKE_LEVELS["extreme"]: level="EXTREME"
elif ratio >= VOLUME_SPIKE_LEVELS["strong"]: level="STRONG"
elif ratio >= VOLUME_SPIKE_LEVELS["caution"]: level="CAUTION"
return {"ratio":ratio,"avg":avg,"last":last,"level":level}

def macd_histogram_trend(macd_hist_series):
try:
last = macd_hist_series.iloc[-3:]
if len(last.dropna())<3: return None
if last.is_monotonic_increasing: return "increasing"
if last.is_monotonic_decreasing: return "decreasing"
return "mixed"
except:
return None
...
if r.iloc[-1]<r.iloc[0]*0.6 and r.std()<r.mean()*0.6: patterns.append("Compression Arc")
# Pulse Stack
...
if (closes.pct_change()>0).sum()>=3 and np.polyfit(range(len(vols)),vols,1)[0]>0: patterns.append("Pulse Stack")
# Trap Funnel
if len(df)>=12:
recent_high=df["close"].tail(24).max()
recent_low=df["close"].tail(24).min()
spike_exists=(df["close"].tail(24)>recent_high*1.02).any()
if spike_exists and df["close"].iloc[-1]<recent_low*0.995: patterns.append("Trap Funnel")
# Sentinel Base
...
"breakout_weak":recent_high*1.02, "breakout_strong":recent_high*1.06,
"breakdown_weak":recent_low*0.98, "breakdown_strong":recent_low*0.95,
...
vol_text="No surge"
if vol["level"]=="EXTREME": vol_text=f"EXTREME volume spike ({vol['ratio']:.2f}x avg)"
elif vol["level"]=="STRONG": vol_text=f"Strong volume spike ({vol['ratio']:.2f}x avg)"
elif vol["level"]=="CAUTION": vol_text=f"Caution volume spike ({vol['ratio']:.2f}x avg)"
...
flips=[]
if price>levels["breakout_weak"]: flips.append("Bullish breakout (weak)")
if price>levels["breakout_strong"]: flips.append("Bullish breakout (strong)")
if price<levels["breakdown_weak"]: flips.append("Bearish breakdown (weak)")
if price<levels["breakdown_strong"]: flips.append("Bearish breakdown (strong)")
if price<levels["danger"]: flips.append("DANGER: price below danger level")
flips_text=", ".join(flips) if flips else "None"
bullish,bearish=50,50
if price>indicators["ma50"]: bullish+=15; bearish-=15
else: bullish-=15; bearish+=15
if indicators["macd_line"]>indicators["macd_signal"]: bullish+=20; bearish-=20
else: bullish-=20; bearish+=20
if indicators["rsi"]<30: bullish+=10; bearish-=10
elif indicators["rsi"]>70: bullish-=10; bearish+=10
if indicators["ma50"]>indicators["ma200"]: bullish+=5; bearish-=5
...
try: update_history_csv(last_row)
except Exception as e: print("CSV update failed:", e)
# send combined Discord message
